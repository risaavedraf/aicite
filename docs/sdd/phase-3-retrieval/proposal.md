...
- CLI commands `harness search` and `harness retrieve`
...
1. `harness search "<query>" --json` returns ranked hits from ready documents only.
2. `harness retrieve "<query>" --json` returns ranked hits with chunk text + metadata.
